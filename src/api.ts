...
export const api = {
  ...
  notificationAuth: () => request<AppState['notification']>('/api/notifications/auth'),
  testNotification: () => request<{ sent: true }>('/api/notifications/test', { method: 'POST' }),
  refresh: (positions?: StoredLpPosition[]) => request<AppState>('/api/refresh', { method: 'POST', body: JSON.stringify(positions ? { positions } : {}) }),
